...
const createChartSVG = (data, placeholder, { svgWidth, svgHeight, elementHeight, scaleWidth, fontSize, minStartDate, maxEndDate, margin, showRelations }) => {
  ...
  const svg = d3.selection(placeholder).append('svg')
  .attr('id', 'gut_ch').attr('width', svgWidth).attr('height', svgHeight);
...
const createGanttChart = (placeholder, data, { elementHeight, sortMode = 'date', showRelations = true, svgOptions }) => {
  // prepare data
